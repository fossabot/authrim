 ...
 */
export const validHexKeyArb = fc.string({
  ...
  for (let patternLen = 2; patternLen <= 8; patternLen++) {
    ...
    const repeated = pattern.repeat(Math.ceil(lowerKey.length / patternLen)).slice(0, lowerKey.length);
 ...
 */
export const nonHexKeyArb = fc.string({
  unit: fc.constantFrom(...HEX_CHARS.split('')),
  minLength: 60,
  maxLength: 63,
}).chain((prefix) =>
  fc.constantFrom('g', 'h', 'z', 'G', 'Z', '!', '@', '#', ' ').map(
    (invalid) => prefix + invalid + HEX_CHARS.slice(0, 63 - prefix.length)
  )
).map((s) => s.slice(0, 64));
 ...
 */
export const jsonPlaintextArb = fc.record({
  sub: fc.uuid(),
  email: fc.tuple(
 ...
 * Provider ID
 */
export const providerIdArb = fc.constantFrom('google', 'github', 'microsoft', 'apple', 'facebook', 'linkedin', 'twitter');

/**
 * Redirect URI
 */
export const redirectUriArb = fc.tuple(
