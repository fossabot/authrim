...
async function discoverTenantIdP(
  email: string,
  env: Env
): Promise<TenantSAMLConfig | null> {
...
app.post('/api/admin/tenants/:tenantId/saml', authenticate, async (c) => {
  ...
  for (const domain of body.email_domains) {
    await c.env.KV.put(`tenant:domain:${domain}`, JSON.stringify({
      tenant_id: tenantId,
      saml_enabled: true
    }));
  }
...
app.get('/authorize', async (c) => {
  ...
  if (loginHint && loginHint.includes('@')) {
    ...
    if (samlConfig) {
      ...
      await c.env.SAML_REQUEST_STORE.put(authnRequest.id, JSON.stringify({
...
async function registerPartnerSP(partnerConfig: PartnerSPConfig) {
  ...
  const sp = await fetch('/api/admin/saml/providers', {
    ...
    body: JSON.stringify({
      ...
      name_id_format: partnerConfig.nameIdFormat ||
...
app.all('/saml/idp/sso', async (c) => {
  ...
  const authnRequest = c.req.method === 'GET'
...
async function issueSAMLResponse(
  ...
  // Sign the assertion
  const signedAssertion = await signSAMLAssertion(
    assertion,
    await getIdPSigningKey(c.env)
  );
  ...
  const signedResponse = await signSAMLResponse(
    response,
    await getIdPSigningKey(c.env)
  );
